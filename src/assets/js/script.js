...
const $projectsBlockItem = document.querySelector('.block-project-info');

Object.entries(projectInfo).forEach(projectInfoEntries => {
  console.log(projectInfoEntries);

  const $projectBlockRow = document.createElement('li');
  $projectBlockRow.className = 'project-block__row';

  const $projectInfoKey = document.createElement('p');
  $projectInfoKey.className = 'project-block__info-key text-bold';
  $projectInfoKey.append(projectInfoEntries[0])

  const $projectInfoValue = document.createElement('p');
  $projectInfoValue.className = 'project-block__info-value text-normal';

  $projectInfoValue.append(projectInfoEntries[1])
  $projectBlockRow.append($projectInfoKey);

  $projectBlockRow.append($projectInfoValue);
  $projectsBlockItem.append($projectBlockRow);
});
